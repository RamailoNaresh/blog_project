...
def get_all_comments(request):
    try:
        data = CommentService.get_all_comments()
...
def get_comment_by_id(request, id):
    try:
        data = CommentService.get_comment_by_id(id)
...
def get_comment_by_post(request, post_id):
    try:
        data = CommentService.get_comment_by_post(post_id)
        serializer = CommentSerializer(data, many = True)
        return Response(serializer.data, status= status.HTTP_200_OK)
    except Exception as e:
        return Response({"Error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
def get_unapproved_comments(request):
    try:
        data = CommentService.get_unapproved_comments()
        serializer = CommentSerializer(data, many  = True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({"Error", str(e)}, status=status.HTTP_400_BAD_REQUEST)